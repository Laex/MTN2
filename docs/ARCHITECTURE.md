...
Разбор аргументов — `StartupPathArgument` и `NoSkiaRequested` в `uUpdater.pas`. Сравнение имён ключей без учёта регистра. `--wait-pid` забирает себя и следующий аргумент, `--no-skia` — только себя; первый оставшийся аргумент считается путём.
