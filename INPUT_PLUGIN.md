...
3. Командная строка Dual Panel — тот же контракт; `submit` маршрутизируется в PTY / внутренние команды ядра по решению Host+ассоциаций. Канон панельной консоли (persistent shell + sync cwd) — [readme.md](readme.md) §5.4, [ARCHITECTURE.md](ARCHITECTURE.md) §8; Tab-complete / вставка путей — roadmap этап 18.
