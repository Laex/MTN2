...
var
  ...
  /// blocked in EnumLogicalDrives -- see Quit.</summary>
  GFullEnumDone: Boolean = True;

/// <summary>An abandoned EnumLogicalDrives worker that wakes during RTL
/// shutdown would allocate on a torn-down heap and crash the exit. ExitProcess
/// stops every other thread first, so use it (skipping finalization, which a
/// test does not need) while that worker is still out.</summary>
procedure Quit(ACode: Integer);
begin
