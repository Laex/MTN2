...
type
  TDraculaTheme = class(TInterfacedObject, IThemeRenderer)
  public
...
const
  ...
  cComment    = TAlphaColor($FF6272A4);
  cCyan       = TAlphaColor($FF8BE9FD);
  cGreen      = TAlphaColor($FF50FA7B);
  cOrange     = TAlphaColor($FFFFB86C);
  cPink       = TAlphaColor($FFFF79C6);
  cPurple     = TAlphaColor($FFBD93F9);
  cRed        = TAlphaColor($FFFF5555);
  cYellow     = TAlphaColor($FFF1FA8C);

  cDesktopBg    = cBackground;
  cWindowBg     = cBackground;
  cText         = cForeground;
  cDialogBg     = cCurrentLine;
  cDialogFg     = cForeground;
  cDialogBorder = cPurple;
  cDirFg        = cCyan;
  cBorderFocus  = cPurple;
  cBorderNormal = cComment;
  cTitleFgFocus = cBackground;
  cTitleBgFocus = cPurple;
  cTitleFgNormal= cComment;
  cTitleBgNormal= cCurrentLine;
  cBtnFg        = cForeground;
  cBtnBg        = cCurrentLine;
  cBtnFgFocus   = cBackground;
  cBtnBgFocus   = cPurple;
  cStatusFg     = cForeground;
  cStatusBg     = cCurrentLine;
  cToolFg       = cForeground;
  cToolBg       = cCurrentLine;
  cToolKeyFg    = cPink;
  cTabActiveFg  = cBackground;
  cTabActiveBg  = cPurple;
  cTabNormalFg  = cComment;
  cTabNormalBg  = cCurrentLine;
  cScrollFg     = cComment;
  cScrollThumb  = cPurple;
  cSelectedFg   = cYellow;
  cSelectedBg   = cWindowBg;
  cCursorFg     = cBackground;
  cCursorBg     = cPink;
  cCursorIdleBg = TAlphaColor($FF3A3C4E);
  cCloseAccent  = cRed;

function TDraculaTheme.DesktopColor: TAlphaColor;
begin
  Result := cDesktopBg;
end;

procedure TDraculaTheme.DrawDesktop(const AGrid: TTerminalGrid; const ABounds: TRectI);
begin
  FillGridRect(AGrid, ABounds.Left, ABounds.Top, ABounds.Right, ABounds.Bottom,
    ' ', cText, cDesktopBg);
end;

procedure TDraculaTheme.DrawWindowFrame(const AGrid: TTerminalGrid;
...
procedure TDraculaTheme.DrawDialogFrame(const AGrid: TTerminalGrid;
  const ABounds: TRectI; const ATitle: string; AState: TThemeWidgetState);
var
  X, Y, W: Integer;
  Cap: string;
...
procedure TDraculaTheme.DrawButton(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const AText: string; AState: TThemeWidgetState);
var
  Fg, Bg: TAlphaColor;
  Cap: string;
...
procedure TDraculaTheme.DrawCheckBox(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const AText: string; AChecked: Boolean; AState: TThemeWidgetState);
var
  Fg, Bg: TAlphaColor;
  Mark: string;
...
procedure TDraculaTheme.DrawRadioBox(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const AText: string; AChecked: Boolean; AState: TThemeWidgetState);
var
  Fg, Bg: TAlphaColor;
  Mark: string;
...
procedure TDraculaTheme.DrawScrollBar(const AGrid: TTerminalGrid; const ABounds: TRectI;
  APosition, AMax: Integer; AVertical: Boolean; AState: TThemeWidgetState);
var
...
procedure TDraculaTheme.DrawTabBar(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const ATabNames: TArray<string>; AActiveTabIndex: Integer;
...
procedure TDraculaTheme.DrawToolBar(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const AItems: TArray<string>; AState: TThemeWidgetState);
var
  I, X, KeyLen: Integer;
  Item, KeyPart: string;
...
procedure TDraculaTheme.DrawStatusLine(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const ASegments: TArray<string>; AState: TThemeWidgetState);
var
  I, X: Integer;
begin
...
procedure TDraculaTheme.DrawMenuBar(const AGrid: TTerminalGrid; const ABounds: TRectI;
  const AMenuText, AClockText: string; AState: TThemeWidgetState);
var
...
procedure TDraculaTheme.DrawPanelFrame(const AGrid: TTerminalGrid; const ABounds: TRectI;
  AActive: Boolean; AState: TThemeWidgetState);
var
  Frame: TAlphaColor;
begin
...
function TDraculaTheme.UsesDoubleLineForActivePanel: Boolean;
begin
  Result := False;
end;

procedure TDraculaTheme.ResolveFileRowColors(AIsDirectory, AIsParent, AIsHidden: Boolean;
  const AFileType: string; ASelected, ACursor, ASideActive: Boolean;
  out AFg, ABg: TAlphaColor);
...
procedure TDraculaTheme.ResolvePanelChromeColors(APart: TPanelChromePart;
...
begin
  ...
  Pal.HeaderFg := cPurple;
  Pal.HeaderBg := cWindowBg;
  Pal.PanelTabActiveFg := cBackground;
  Pal.PanelTabActiveBg := cPurple;
...
procedure TDraculaTheme.ResolveMarkdownStyleColors(AKind: TMdSpanKind;
  out AFg, ABg: TAlphaColor; out AAttr: TCharCellAttributes);
begin
  AAttr := [];
  ABg := cWindowBg;
  case AKind of
    mskH1: begin AFg := cPurple; AAttr := [ccaBold]; end;
    mskH2: begin AFg := cYellow; AAttr := [ccaBold]; end;
    mskH3to6: begin AFg := cGreen; AAttr := [ccaBold]; end;
    mskBold: begin AFg := cForeground; AAttr := [ccaBold]; end;
    mskItalic: AFg := cPink;
    mskBoldItalic: begin AFg := cPink; AAttr := [ccaBold]; end;
    mskStrike: AFg := cComment;
    mskInlineCode: begin AFg := cGreen; ABg := cCurrentLine; end;
    mskCodeBlock: begin AFg := cForeground; ABg := cCurrentLine; end;
    mskQuote: AFg := cComment;
    mskListMarker: begin AFg := cYellow; AAttr := [ccaBold]; end;
    mskHRule: AFg := cComment;
    mskLink: AFg := cCyan;
    mskImageMarker: AFg := cComment;
    mskTableBorder: AFg := cComment;
...
procedure TDraculaTheme.ResolveEditorColors(out AColors: TEditorThemeColors);
begin
